...
async function findResultDirectories(path: string): Promise<string[]> {
  const directories = [];
  ...
  for (const entry of await readdir(path, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      directories.push(...(await findResultDirectories(child)));
...
export async function readRefreshResults(resultsDirectory: string) {
  return Promise.all(
    (await findResultDirectories(resultsDirectory)).map(async (directory) => ({
      directory,
...
async function main() {
  ...
  try {
    const resultEntries = await readRefreshResults(resultsDirectory);
    const results = resultEntries.map(({ result }) => result);

    validateRefreshResults(plan, snapshot.revision, results);

    for (const batch of plan.batches) {
      const entry = resultEntries.find(({ result }) => result.id === batch.id)!;
      const { directory, result } = entry;
